import { removeChatUsers } from "@services/api/chat.service";
...
export class ChatUtils {
    ...
    static messageData({
        ...
        const chatConversationId = find(chatMessages, (chat) => chat.receiver === searchParamsId || chat.senderId === searchParamsId)
    ...
    static updateSelectedChatUser({
        ...
        if (chatMessageList) {
            ...
            const sender = find(this.chatUsers, (user) => user.userOne.toLowerCase() === profile.username && user.userTwo.toLowerCase() === username)
            if (sender) {
